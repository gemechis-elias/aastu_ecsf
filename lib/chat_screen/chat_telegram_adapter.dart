      ...
      for (final word in words) {
        if (hashtagRegex.hasMatch(word)) {
          ...
          textSpans.add(getStyledText(
              word, TextStyle(color: Color.fromARGB(255, 11, 58, 97))));
        } else if (usernameRegex.hasMatch(word)) {
          // If the word matches the username pattern, add a blue colored text span
          textSpans.add(getStyledText(
              word, TextStyle(color: Color.fromARGB(255, 14, 58, 94))));
        } else if (Uri.parse(word).isAbsolute) {
          // If the word is a valid URL, add a blue colored text span
          textSpans.add(getStyledText(
              word, TextStyle(color: Color.fromARGB(255, 16, 63, 102))));
        } else {
          // Otherwise, add the text without any styling
          textSpans.add(getStyledText(word, TextStyle(color: Colors.black)));
        }
        // Add a space after each word
        textSpans.add(getStyledText(' ', TextStyle(color: Colors.black)));
      }
      ...
      children: <Widget>[
        Card(
            ...
            color: isMe
                ...
                : const Color.fromARGB(255, 167, 150, 112),
            elevation: 1,
            child: Padding(
